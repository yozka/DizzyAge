...
//-----------------------------------------------------------------------------
void*	ym_fopen		( char* filename )						{ return (void*)fopen( filename, "rb" ); }
int		ym_fclose		( void* file )							{ return fclose( (FILE*)file ); }
int		ym_fseek		( void* file, int pos, int mode )		{ return fseek( (FILE*)file, pos, mode ); }
int		ym_ftell		( void* file )							{ return ftell( (FILE*)file ); }
int		ym_fread		( void* buffer, int size, void* file )	{ return fread( buffer, 1, size, (FILE*)file ); }

ymFileSystem ymFS = { ym_fopen, ym_fclose, ym_fseek, ym_ftell, ym_fread };
void	ymSetFileSystem	( ymFileSystem* fs )					{ if(fs!=NULL) ymFS = *fs; }
...
static ymint	fileSizeGet(void *h)
 {
 ymint size;
 ymint old;

		old = ymFS.m_ftell(h);
		ymFS.m_fseek(h,0,SEEK_END);
		size = ymFS.m_ftell(h);
		ymFS.m_fseek(h,old,SEEK_SET);
		return size;
 }
...
{
void	*in;
		if( !(ymFS.m_fopen && ymFS.m_fclose && ymFS.m_fseek && ymFS.m_ftell && ymFS.m_fread) ) return YMFALSE;
		...
		//---------------------------------------------------
		if (ymFS.m_fread(pBigMalloc,fileSize,in)!=(size_t)fileSize)
		{
			free(pBigMalloc);
