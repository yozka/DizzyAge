...
struct ymFileSystem
{
	void*	(*m_fopen)	( char* filename);
	int		(*m_fclose)	( void* file );
	int		(*m_fseek)	( void* file, int pos, int mode );
	int		(*m_ftell)	( void* file );
	int		(*m_fread)	( void* buffer, int size, void* file );
};
void ymSetFileSystem( ymFileSystem* fs );
